...
abstract class BaseHostActivity : Activity(), ModuleActivity {
    ...
    private fun injectModuleResourcesFallback(context: Context) {
        runCatching {
            // 检查当前 Context.getResources() 是否已包含模块资源表
            val testId = R.drawable.bg_card
            val testResult = runCatching { resources.getDrawable(testId, theme) }
            if (testResult.isSuccess) {
                YLog.info("BaseHostActivity: module resources already available, skip fallback")
                return
            }
            YLog.info("BaseHostActivity: module resources NOT available (${testResult.exceptionOrNull()?.message}), applying fallback")

            // 创建模块 Resources 并用 ContextWrapper 注入 Context 链
            val moduleRes = getModuleResources(context) ?: return
            val verifyResult = runCatching { moduleRes.getDrawable(testId, theme) }
            if (verifyResult.isFailure) {
                YLog.error("BaseHostActivity: module Resources cannot load module drawable: ${verifyResult.exceptionOrNull()?.message}")
                return
            }
            // 重设 mBase 为包含模块 Resources 的 ContextWrapper
            Activity::class.java.getDeclaredField("mBase").apply {
                isAccessible = true
                set(this@BaseHostActivity, ModuleResourcesContextWrapper(context, moduleRes))
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        // 在 delegate.onCreate 之前注入模块资源，避免 delegate.onCreate 调用 setTheme 时
        // 因模块资源未注入导致资源 ID 被错误解析
        if (moduleTheme != -1) {
            try {
                // 先注入模块资源
                ...
                YLog.error("BaseHostActivity: pre-setTheme failed: ${e.message}")
            }
        }
